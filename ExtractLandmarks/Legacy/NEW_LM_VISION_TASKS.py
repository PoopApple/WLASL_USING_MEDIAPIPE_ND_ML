    ...
    while cap.isOpened():
        ...
        if pose_landmarker_result.pose_landmarks:
            ...
            for landmark_index, i in enumerate(needed_poses):
                xx = arr[i].x - CENTER_OF_SHOULDER[0]
                yy = arr[i].y - CENTER_OF_SHOULDER[1]
                zz = arr[i].z - CENTER_OF_SHOULDER[2]
                vv = arr[i].visibility
            ...
            if dopythonlists:
                landmark_for_one_frame.extend(
                    [
                        [
                            (arr[i].x - CENTER_OF_SHOULDER[0]) / SHOULDER_LENGTH,
                            (arr[i].y - CENTER_OF_SHOULDER[1]) / TORSO_HEIGHT,
                            arr[i].z - CENTER_OF_SHOULDER[2],
                            arr[i].visibility,
                        ]
            ...
            if filename.lower().endswith(".mp4"):
                ...
                if doflipalso:
                    ...
                    output_path = os.path.join(
                        output_word_folder, filename[:-4] + "_flipped.npy"
                    )
                    if printdebug:
                        print(input_path)
                        print(output_path)
