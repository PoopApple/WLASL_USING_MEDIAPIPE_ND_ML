...
def doshit(
    vid_path="./gte9_dataset/animal/02583.mp4",
    showvid=False,
    dopythonlists=False,
    ...
    while cap.isOpened():
        ...
        pose_landmarker_result = pose_landmarker.detect_for_video(mp_image, curr_frame_ms)
        ...
        if pose_landmarker_result.pose_landmarks:
            poseLMs = pose_landmarker_result.pose_landmarks[0]
            ...
            for landmark_index, i in enumerate(needed_poses):
                xx = poseLMs[i].x - CENTER_OF_SHOULDER[0]
                yy = poseLMs[i].y - CENTER_OF_SHOULDER[1]
                zz = poseLMs[i].z - CENTER_OF_SHOULDER[2]
                vv = poseLMs[i].visibility
            ...
            if showvid:
                annotated_image = draw_pose_landmarks_on_image(image, pose_landmarker_result)
        ...
        hand_landmarker_result = hand_landmarker.detect_for_video(mp_image, curr_frame_ms)
            ...
            for idx,handedd in enumerate(hand_landmarker_result.handedness):
                ...
                for i in range(21):
                    NP_LANDMARK_ALL_FRAMES[frame_index][landmark_index + i] = [
                        (handLMs[i].x - CENTER_OF_SHOULDER[0])/SHOULDER_LENGTH,
                        (handLMs[i].y - CENTER_OF_SHOULDER[1])/TORSO_HEIGHT,
                        handLMs[i].z - CENTER_OF_SHOULDER[2],
                        1,
                    ]
                ...
                if dopythonlists:
                    hand_landmarks_list[lbl] = [
                        [
                            handLMs[ind].x - CENTER_OF_SHOULDER[0],
                            handLMs[ind].y - CENTER_OF_SHOULDER[1],
                            handLMs[ind].z - CENTER_OF_SHOULDER[2],
                            1,
                        ]
                        for ind in range(21)
                    ]
                ...
                if showvid:
                    annotated_image = draw_hands_landmarks_on_image(annotated_image, hand_landmarker_result)
    ...
    def stickyhands(NP_LANDMARK_ALL_FRAMES, total_num_frames=total_num_frames):
