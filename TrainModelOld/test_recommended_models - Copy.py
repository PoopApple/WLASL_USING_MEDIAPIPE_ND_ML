...
def build_bigru_balanced_regularization(num_classes, name="BiGRU_Balanced_Regularization"):
    """
    ⭐ RECOMMENDED FOR SMALL DATASETS ⭐
    Bi-GRU model with VERY STRONG regularization (FAST VERSION - no recurrent_dropout)
    Designed to combat SEVERE overfitting on tiny datasets (204 classes, ~10 samples/class)
    Expected accuracy: 40-55% with augmentation (improved from 39%)
    Training speed: ~35-50ms/step (5-8x faster than version with recurrent_dropout)
    """
    model = models.Sequential([
        layers.Input(shape=(SEQUENCE_LENGTH, FEATURE_DIM)),
        
        # Slightly increased capacity for better fit
...
def main():
    ...
    plot_results(history, cm, label_encoder, "BiGRU_Balanced_Regularization", bin_conf)
    save_top5_predictions(y_pred_probs, y_true, label_encoder, "BiGRU_Balanced_Regularization")
    # Capture final train accuracy
    train_acc = float(history.history.get('accuracy', [None])[-1]) if 'accuracy' in history.history else None
    results['BiGRU_Balanced_Regularization'] = {
        'accuracy': acc,
        'loss': loss,
        'top5': float(top5) if top5 is not None else None,
        'train_accuracy': train_acc
        ...
        if os.path.exists(INCREMENTAL_JSON):
            with open(INCREMENTAL_JSON, 'r') as jf:
                try:
                    ...
                    existing = {}
        existing["BiGRU_Balanced_Regularization"] = results['BiGRU_Balanced_Regularization']
        with open(INCREMENTAL_JSON, 'w') as jf:
            json.dump(existing, jf, indent=2)
        print(f"✅ Incremental results saved to {INCREMENTAL_JSON}")
    except Exception as e:
